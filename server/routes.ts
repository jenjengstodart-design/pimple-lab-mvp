...
const SUPERDRUG_PRODUCTS = [
  { id: "1", name: "CeraVe SA Smoothing Cleanser", category: "salicylic acid", price: "£14.00", link:
"https://www.superdrug.com" },
  { id: "2", name: "The Ordinary Salicylic Acid 2% Solution", category: "salicylic acid", price: "£6.30", link:
"https://www.superdrug.com" },
  { id: "3", name: "Me+ Salicylic Acid & Ceramide Cleanser", category: "salicylic acid", price: "£7.99", link:
"https://www.superdrug.com" },
  { id: "4", name: "Acnecide Face Gel Spot Treatment 5%", category: "benzoyl peroxide", price: "£11.99", link:
"https://www.superdrug.com" },
  { id: "5", name: "Acnecide Face Wash 5%", category: "benzoyl peroxide", price: "£11.99", link:
"https://www.superdrug.com" },
  { id: "6", name: "Cetaphil Gentle Skin Cleanser", category: "gentle cleanser", price: "£10.00", link:
"https://www.superdrug.com" },
  { id: "7", name: "Simple Water Boost Micellar Gel Wash", category: "gentle cleanser", price: "£5.99", link:
"https://www.superdrug.com" },
  { id: "8", name: "Neutrogena Clear & Defend Oil-Free Moisturiser", category: "oil-free moisturiser", price: "£7.50",
 link: "https://www.superdrug.com" },
  { id: "9", name: "e.l.f. Skin Superhydrate Moisturiser", category: "oil-free moisturiser", price: "£12.00", link:
"https://www.superdrug.com" },
  { id: "10", name: "Nizoral Anti-Dandruff Shampoo (for fungal acne)", category: "antifungal-friendly", price:
"£6.50", link: "https://www.superdrug.com" },
  { id: "11", name: "Starface Hydro-Stars", category: "spot treatment", price: "£11.99", link:
"https://www.superdrug.com" },
  { id: "12", name: "Byoma Creamy Jelly Cleanser", category: "gentle cleanser", price: "£9.99", link:
"https://www.superdrug.com" },
];
...
async function analyze_with_medgemma(imageUrl: string, context: any): Promise<{
  ...
  if (!process.env.MEDGEMMA_ENDPOINT) throw new Error("MEDGEMMA_ENDPOINT is not set");

  const imagePath = path.join(process.cwd(), imageUrl);
  const imageBuffer = fs.readFileSync(imagePath);
  const base64Image = imageBuffer.toString("base64");

  const prompt = `Analyze this skin image for acne. Respond ONLY with valid JSON (no markdown, no code blocks) with
exactly these keys:
{
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.post(api.experiments.create.path, async (req, res) => {
    try {
      ...
      if (err instanceof z.ZodError) return res.status(400).json({ message: err.errors[0].message, field:
...
err.errors[0].path.join('.') });
  ...
  app.post(api.experiments.followUp.path, async (req, res) => {
    try {
      ...
      const imageUrl = saveBase64Image(input.imageBase64);
      const experiment = await storage.updateExperiment(Number(req.params.id), { followUpImageUrl: imageUrl, outcome:
input.outcome });
      if (!experiment) return res.status(404).json({ message: "Experiment not found" });
      res.json(experiment);
    } catch (err) {
